...
class OverallWinsCalculator(YearCalculator):
    """
    ...
    @classmethod
    @validateYear
    def getOverallWins(cls, year: Year, **kwargs) -> dict[str, Optional[Deci]]:
        """
        ...
        teamIdsAndScores = dict()

        for i in range(filters.weekNumberStart - 1, filters.weekNumberEnd):
            week = year.weeks[i]
            teamIdsAndScores[week.weekNumber] = list()
            for matchup in week.matchups:
                if matchup.matchupType in filters.includeMatchupTypes:
                    teamIdsAndScores[week.weekNumber].append((matchup.teamAId, matchup.teamAScore))
                    teamIdsAndScores[week.weekNumber].append((matchup.teamBId, matchup.teamBScore))
        ...
        for i in range(filters.weekNumberStart - 1, filters.weekNumberEnd):
            week = year.weeks[i].weekNumber
            for teamIdAndScore in teamIdsAndScores[week]:
                scoresBeat, scoresTied = getNumberOfScoresBeatAndTied(teamIdAndScore[1], YearNavigator.getAllScoresInWeek(year, week))
